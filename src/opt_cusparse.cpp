...
void OptimizeProblem (const SpMat &A, const Vec &x, SpMatOpt &A_opt, VecOpt &x_opt) {
    ...
    CUDA_SAFE_CALL(cudaMemcpy((void *)cuda_ptr, A_opt.ptr, (A.nNnz + 1) * sizeof(int), cudaMemcpyHostToDevice));
    CUDA_SAFE_CALL(cudaMemcpy((void *)cuda_idx, A_opt.idx, A.nNnz * sizeof(int), cudaMemcpyHostToDevice));
    CUDA_SAFE_CALL(cudaMemcpy((void *)cuda_val, A_opt.val, A.nNnz * sizeof(double), cudaMemcpyHostToDevice));

    CUDA_SAFE_CALL(cudaMalloc((void**)&cuda_x, A.nCol * sizeof(double)));
    CUDA_SAFE_CALL(cudaMalloc((void**)&cuda_y, A.nRow * sizeof(double)));
