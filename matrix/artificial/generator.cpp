...
M band_matrix (int N, int param) {
    ...
    for (int i = 0; i < param; i++) {
        int r = rand()%N;
        int c = rand()%N;
        while (abs(r-c) < width || s.count(P(r, c))) {
            r = rand()%N;
            c = rand()%N;
        }
        s.insert(P(r, c));
...
M unbalance_matrix (int N, int param) {
    M m;
    int H = param/N + (param%N==0?0:1);
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < N; j++) {
            if (param == 0) continue;
            param--;
...
int main (int argc, char* argv[]) {
    ...
    else if (strcmp(argv[1], "unbalance") == 0) print_matrix(N, unbalance_matrix(N, param));
